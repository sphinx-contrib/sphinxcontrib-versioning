...
def pre_build(local_root, conf_rel_paths, versions, root_ref, overflow):
    ...
    for sha in {r['sha'] for r in versions.remotes}:
        ...
        export(local_root, conf_rel_paths, sha, target)

    # Build root ref.
    with TempDir() as temp_dir:
        log.debug('Building root ref (before setting URLs) in temporary directory: %s', temp_dir)
        build(os.path.join(exported_root, root_remote['sha']), temp_dir, versions, root_ref, overflow)
    ...
    for remote in versions.remotes:
        ...
        config = read_config(os.path.join(exported_root, remote['sha']), remote['name'], overflow)
